...
SELECT LASTNAME, FIRSTNAME FROM EMPLOYEES WHERE EMPLOYEENUMBER IN (SELECT EMPLOYEENUMBER, COUNT(REPORTSTO) FROM EMPLOYEES WHERE EMPLOYEENUMBER = EMPLOYEES.REPORTSTO GROUP BY EMPLOYEENUMBER);

-- 45. Select all employees who work for the manager that manages the greatest number of
-- employee (12)

SELECT COUNT(REPORTSTO) FROM EMPLOYEES WHERE REPORTSTO IN (SELECT );

-- 46. List all employees that have the same last name. Make sure each combination is listed
-- only once (5)

SELECT CUSTOMERNAME, CONTACTFIRSTNAME, CONTACTLASTNAME FROM CUSTOMERS WHERE CUSTOMERNAME= (SELECT MIN(CUSTOMERNAME) FROM CUSTOMERS);

SELECT one.CUSTOMERNAME AS "First Customer", two.CUSTOMERNAME as "Other Customer", PAYMENTDATE FROM (

        (SELECT CUSTOMERNUMBER, CUSTOMERNAME, PAYMENTDATE FROM CUSTOMERS INNER JOIN PAYMENTS USING (CUSTOMERNUMBER)) one

INNER JOIN

        (SELECT CUSTOMERNUMBER, CUSTOMERNAME, PAYMENTDATE FROM CUSTOMERS INNER JOIN PAYMENTS USING (CUSTOMERNUMBER)) two
